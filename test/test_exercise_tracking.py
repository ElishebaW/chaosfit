#!/usr/bin/env python3
"""
Integration test to verify exercise tracking and summary display.
Tests that multiple exercises are properly saved to session state
and displayed correctly in the summary page.
"""
...
async def test_exercise_tracking():
    """Test that exercises are properly tracked in session state."""
    print("🧪 Testing Exercise Tracking Integration")
    print("=" * 50)
    ...
    print(f"📊 Initial state - reps: {state.cumulative_rep_count}, corrections: {len(state.form_corrections)}")
    ...
    session_manager.record_session_summary(
        session_id=session_id,
        user_id="test-user",
        exercise_type=final_state.current_exercise,
        rep_count=final_state.cumulative_rep_count,
        form_corrections=final_state.form_corrections,
...
if __name__ == "__main__":
    asyncio.run(test_exercise_tracking())
