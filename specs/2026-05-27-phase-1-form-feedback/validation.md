...
## Checklist
- [ ] Firestore session data queried; top failure patterns documented in `evals/dataset.json`
- [ ] `eval_correction_specificity` eval threshold defined and agreed
- [ ] New eval cases added for timing failures (early trigger, repeated correction) and squat/push-up/plank landmark errors
- [ ] `eval_correction_specificity` passes at or above the defined threshold on all new cases
- [ ] `eval_interruption_integrity` and `eval_rep_count_accuracy` continue to pass (no regression)
- [ ] CI runs `evals/run_evals.py` on PRs touching `backend/live_agent/` or `evals/`
